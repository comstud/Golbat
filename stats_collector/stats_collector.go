...
type StatsCollector interface {
	...
	IncDecodeEncounter(status, messages string, areas []geo.AreaName)
	IncDecodeDiskEncounter(status, message string, areas []geo.AreaName)
	IncDecodeEncounterType(typ, status, messages string, areas []geo.AreaName)
	IncDecodeQuest(status, message string, areas []geo.AreaName)
