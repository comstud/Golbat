...
func main() {
	...
	if cfg.Prometheus.Enabled {
		...
		statsCollector = stats_collector.NewPrometheusCollector()
		p := ginprom.New(
			ginprom.Engine(r),
			ginprom.Subsystem("gin"),
			ginprom.Path("/metrics"),
			ginprom.Token(cfg.Prometheus.Token),
			ginprom.BucketSize(cfg.Prometheus.BucketSize),
		)
		r.Use(p.Instrument())
	} else {
...
func decodeDiskEncounter(ctx context.Context, protoData *raw_decoder.Proto) (bool, string) {
	...
	return true, decoder.UpdatePokemonRecordWithDiskEncounterProto(ctx, dbDetails, decodedEncounterInfo)
}
